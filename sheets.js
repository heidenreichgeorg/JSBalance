...
function load(sessionId,phaseOneFunction) {
    ...
    } else { console.error("Sheets.js UPDATE("+sessionId+") NO  SESSION"); }
...
async function saveLogT(client,logT) {
    console.log("sheets.js saveLogT Saving(JSON)");        
    //log("saveLogT");  
    const data = JSON.stringify(logT);
    if(data) {
        var pSave = fs.writeFileSync(jsonLogf(client), data, {'encoding':'utf8'}, (err) => { // was latin1 GH20211120
            if (err) {
                console.log("sheets.js saveLogT: "+err);          
                throw err;
            }
            console.log("sheets.js saveLogT Saving("+data+")");          
        });
    }
...
function readCSVFile(foundPath) {
    var aoaCells = [];
    if(foundPath) {
        if(debug) console.log("readCSVFile reading "+foundPath);
        try {
            ...
            console.error('server.js readCSVFile:'+err);
        }
    }
    return aoaCells;
}
...
// SECURITY - SANITIZE INPUT
    ...
    for(;row<csvLines.length && row<SY_MAXROWS;row++) { 
        ...
        if(rawLine) {
            ...
            if(parseInt(cells[0])>0) console.log('sheets.js readNumeric storeCell '+rawLine);
            ...
            if(keep) { // avoid empty rows
                ...
                console.log('readNumeric('+row+') SKIP '+rawLine);
            }
        }
    }
    return aoaCells;
...
function create(client,year,time,remote,sessionId) {
    ...
    try {
        ...
        console.log("getSheet reads MAIN JSON sheetCells#"+session.sheetCells.length);

    }  catch (err) {
        console.error('sheets.js getSheet MAIN ERROR:'+err);
        console.error('sheets.js getSheet MAIN --->getFromFile');
        console.log('sheets.js getFromFile MAIN used '+sFile);
    }

    var lFile=jsonLogf(client);
    try {
        console.log("Sheets.getSheet LOGF "+lFile);
        var logT = JSON.parse(fs.readFileSync(lFile,{'encoding':'utf8'})); // was latin1 GH20211120
        if(debug>2) console.log("getSheet reads LOGF JSON content: "+JSON.stringify(logT));
        session.logT = logT;
        if(debug>0) console.log("getSheet reads LOGF JSON logt#"+session.logT.length);
        
    }  catch (err) {
        session.logT = { };
        console.error('sheets.js getSheet LOGT ERROR:'+err);
        console.log('sheets.js getFromFile LOGT used '+lFile);
    }
    ...
    try {
        ...
        session.ADDRT = { };
        console.error('sheets.js getSheet ADDRT ERROR:'+err);
        console.ADDR('sheets.js getFromFile ADDRT used '+lFile);
    }
    */
   // bootstrap
    session.addrT = { 
    ...
    setTimeout(function(){
        ...
        if(sFile && sFile.length>3) {
            ...
            console.log('sheets.js getSheet finds '+sFile);
        } else console.log('sheets.js getSheet finds NO sheetFile');
        if(!session.sheetCells || session.sheetCells.length<H_LEN) {
