   ...
   function cents2EU(cents) {
    ...
    var milleNum = parseInt(cents/100000);
    var milleStr = milleNum>0 ? milleStr=milleNum.toString()+"." : "";
    cents-=(milleNum*100000);

    var euroNum = parseInt(cents/100);
    var euroStr = milleNum>0  ? euroNum.toString().padStart(3,'0') : euroNum.toString();
    cents-=(euroNum*100);

    return sign + milleStr + euroStr+","+(parseInt(cents%100).toString().padStart(2,'0'));
}
